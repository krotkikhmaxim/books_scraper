...
def init_http(book_url):
    ...
    try:
        response = requests.get(book_url, timeout=100)
...
def init_parsing(response):
    ...
    try:
        ...
        raise BookPageParseError(f"Ошибка парсинга HTML: {e}") from e
...
def make_data(soup, base_url, class_table, must_have_data):
    ...
    if len(category_links) > 2:
        data['category'] = category_links[2].text.strip()
    else:
        data['category'] = None
    ...
    data['image_url'] = base_url + image['src'].replace('../..', ''
                                                        ) if image else None

    set_data = {key for key, value in data.items() if value is None}

    none_data = set(must_have_data) & set_data

    if none_data:
        msg_none_data = f"Обязательные поля не заполнены: {', '.join(none_data)}"
        raise BookDataNotFoundError(msg_none_data)
    return data
...
def validation_get_book_data(func):
    ...
    def wrapper(
        ...
        try:
            ...
            text_e = f"Непредвиденная ошибка при обработке страницы: {e}"
            raise BookPageParseError(text_e) from e
        return res
    return wrapper
...
def scrape_books(
    ...
    Args:
        is_save (bool): Сохранять ли данные в файл.
            По умолчанию True

        base_url (str): Шаблон URL для pagination.
            По умолчанию URL books.toscrape.com

        page_num (int): Номер страницы для начала парсинга.
            По умолчанию 1

        class_list (str): CSS класс для поиска контейнеров с книгами.
            По умолчанию "image_container"

        file_name (str): Имя файла для сохранения данных.
            По умолчанию 'books_data.txt'

        format_data (callable): Функция форматирования данных перед сохранением.
            По умолчанию str

        stop (int): Номер страницы на которой остановиться.
            По умолчанию 2
    ...
    while page_num <= stop:
        ...
        try:
            ...
            for link in soup:
                ...
                full_url = start_url+link.find('a', href=True)['href']
            ...
            if is_save:
                directory = Path(folder)  # относительный путь
